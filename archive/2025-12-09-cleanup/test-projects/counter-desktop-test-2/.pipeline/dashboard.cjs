 ...
 * Sends heartbeat messages to orchestrator via SendKeys using PID + HWND.
 */
...
const ORCHESTRATOR_PID = process.argv[3] || null; // PID of orchestrator's WindowsTerminal process
...
function sendHeartbeat() {
  ...
  // Message to send (simple format, avoid special chars)
  const message = `HEARTBEAT ${heartbeatCount} Phase ${currentPhase} ${todoSummary}`;

  // Log heartbeat attempt
  console.log(`\n[HEARTBEAT ${heartbeatCount}] Sending to PID ${ORCHESTRATOR_PID}: Phase ${currentPhase}, ${todoSummary}`);

  // Send message to orchestrator using:
  // 1. Get MainWindowHandle (HWND) from PID
  // 2. Use SetForegroundWindow(HWND) to activate
  // 3. SendKeys to type the message
  try {
...
try {
    $proc = Get-Process -Id ${ORCHESTRATOR_PID} -ErrorAction Stop
    ...
    if ($hwnd -ne [IntPtr]::Zero) {
        ...
        if ($activated) {
            ...
            Write-Host 'SendKeys SUCCESS to PID ${ORCHESTRATOR_PID} (HWND: ' + $hwnd + ')'
        } else {
            Write-Host 'SetForegroundWindow FAILED for HWND: ' + $hwnd
        }
    } else {
        Write-Host 'MainWindowHandle is zero - window not found for PID ${ORCHESTRATOR_PID}'
    }
} catch {
    Write-Host ('SendKeys FAILED: ' + $_.Exception.Message)
}`;

    fs.writeFileSync(scriptPath, psScript);

    const result = spawn('powershell.exe', ['-ExecutionPolicy', 'Bypass', '-File', scriptPath], {
      stdio: 'inherit',
      detached: false,
      shell: false
    });
...
function main() {
  ...
  if (ORCHESTRATOR_PID) {
    ...
    console.log(`Heartbeat: every ${HEARTBEAT_INTERVAL / 1000}s (via PID->HWND->SetForegroundWindow->SendKeys)`);
  } else {
  ...
  // Heartbeat loop (orchestrator messages via SendKeys)
  if (ORCHESTRATOR_PID) {
    setInterval(sendHeartbeat, HEARTBEAT_INTERVAL);
    // Send first heartbeat after 5 seconds for testing
