# Pipeline Orchestrator v9.0 (Desktop)
...
- Enhanced Gate 1: Interaction helpers, smoke test skeleton checks
- Enhanced Gate 2: Quality audit report, completeness pairs, no placeholders
- Completeness pairs verification across all phases
- Onboarding level tracking in manifest
- Worker base rules extracted to worker-base-desktop-v9.0.md
- Orchestrator injects base rules before phase commands via BaseRules parameter
- Dashboard auto-calculates token breakdown (regular vs cached tokens/cost)
...
5. Run: `claude`
6. Type: `/orchestrator-desktop-v9.0`
...
**Worker Injection:** When spawning workers, always pass `-BaseRules "/worker-base-desktop-v9.0"` to inject base rules.
...
If this command was invoked as `/orchestrator-desktop-v9.0 AUTO-RESUME-NO-QUESTION`, skip the question below and go directly to **"Resume Flow"**.
...
| Worker dead -> Respawn | Spawn worker with BaseRules, then section 7 |
...
Use **AskUserQuestion** with TWO questions (Stack is always Desktop for v9.0):
...
**Enhanced in v9.0 with infrastructure checks.**
...
if [ "$PHASE" = "3" ]; then
  ...
  # CHECK 2: No direct API calls in E2E tests (NEW v9.0)
  ...
  # CHECK 3: Interaction helpers exist (NEW v9.0)
  ...
  # CHECK 4: Smoke test skeleton exists (NEW v9.0)
...
**Enhanced in v9.0 with quality audit checks.**
...
if [ "$RUN_GATE2" = "true" ]; then
  ...
  # CHECK 5: Buttons without onClick (NEW v9.0)
  ...
  # CHECK 6: Smoke test items all passed (NEW v9.0)
...
### 8.4 Phase 5 Completion: Save Quality Audit Results (v9.0)
...
- **Always use BaseRules parameter** - inject base rules into workers
- **Always store all 6 token fields** - regularTokens, regularCost, cachedTokens, cachedCost, tokens, cost
- **Version 9.0 is self-contained** - no references to other versions
- **Gate 1 now checks for interaction helpers and smoke test skeleton**
- **Gate 2 now checks for quality audit and smoke test pass**
- **Onboarding level is tracked in manifest**
- **Quality audit results are saved to manifest**
