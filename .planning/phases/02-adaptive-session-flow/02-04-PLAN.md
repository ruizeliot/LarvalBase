...
<task type="auto">
  <name>Task 2: Add Live Canvas MCP Integration section to CLAUDE.md</name>
  <files>.claude/CLAUDE.md</files>
  <action>
Insert a new major section "# Live Canvas MCP Integration" before the "# Phase 1: Brainstorm & User Stories" section. Include:
...
**In DIVERGE mode (discover/develop):**
...
**In CONVERGE mode (define/deliver):**
...
2. All 5 subsections present (Tools, Session Phase, Notes, Engagement, Technique Switching)
3. File still valid markdown
4. grep -c "get_session_phase" returns at least 2 occurrences
5. grep -c "create_mindmap" returns at least 2 occurrences
  </verify>
  <done>CLAUDE.md updated with comprehensive Live Canvas MCP behavioral instructions</done>
</task>

<task type="auto">
  <name>Task 3: Commit CLAUDE.md changes</name>
  <files>.claude/CLAUDE.md</files>
  <action>
...
- Session phase awareness (get_session_phase calls)
- Diverge vs converge behavioral rules
...
2. Instructions cover all Phase 2 infrastructure:
   ...
   - Session phase awareness (get_session_phase)
   - Notes discipline (append_notes)
   - Engagement response patterns
   - Technique switching
3. File is valid markdown with consistent formatting
...
- Claude (when running brainstorm) will know to:
  ...
  - Check and adapt to session phase
