...
function render() {
  ...
  if (manifest.createdAt) {
    ...
    durationLine = 'Started: ' + startDate + '  │  Duration: ' + duration;
  }
  ...
  for (const phase of phases) {
    ...
    if (phase === '4' && manifest.epics && manifest.epics.length > 0) {
      for (const epic of manifest.epics) {
        ...
        lines.push('\x1b[36m║\x1b[0m      ' + epicIcon + ' ' + epicName.padEnd(42) + ' \x1b[36m║\x1b[0m');
      }
    }
  }
  ...
  // Heartbeat countdown
  if (ORCHESTRATOR_PID && nextHeartbeatTime && manifest.status !== 'complete' && manifest.status !== 'paused') {
    const countdown = nextHeartbeatTime - Date.now();
    const countdownStr = countdown > 0 ? formatCountdown(countdown) : 'NOW';
    lines.push('\x1b[36m║\x1b[0m  \x1b[1mHEARTBEAT\x1b[0m  Next in: ' + countdownStr.padEnd(30) + ' \x1b[36m║\x1b[0m');
    lines.push('\x1b[36m║\x1b[0m                                                      \x1b[36m║\x1b[0m');
  }
...
function sendHeartbeat() {
  ...
  nextHeartbeatTime = lastHeartbeatTime + HEARTBEAT_INTERVAL;
...
function main() {
  ...
  if (ORCHESTRATOR_PID) console.log('Heartbeat: every 3min to PID ' + ORCHESTRATOR_PID + ' (WriteConsoleInput)');
  console.log('');
  ...
  if (ORCHESTRATOR_PID) {
    // Schedule first heartbeat after 1 second
    nextHeartbeatTime = Date.now() + 1000;
    setTimeout(sendHeartbeat, 1000);
    // Then heartbeat every 3 minutes
