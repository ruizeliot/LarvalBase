...
// ============================================================================
// Supervisor Transcript Injection Tests
// ============================================================================

describe('Supervisor Transcript Injection', () => {
  describe('when a todo completes', () => {
    test('should call sendSupervisorCheck with transcript slice', async () => {
      // This test verifies the bug fix: supervisor should receive transcripts
      // when todos complete, not just at phase end

      const { sendSupervisorCheck } = require('../supervisor-check.cjs');

      // Mock the inject function
      const mockInject = jest.fn().mockResolvedValue(true);
      ...
      const context = {
        ...
        transcriptSlice: 'Sample transcript content...',
        injectToRole: mockInject
      };

      await sendSupervisorCheck(completedTodo, context);

      // Verify supervisor was called with the right format
      expect(mockInject).toHaveBeenCalledWith(
        __dirname,
        'supervisor',
        expect.stringContaining('CHECK TODO:')
      );
      expect(mockInject).toHaveBeenCalledWith(
        __dirname,
        'supervisor',
    ...
    test('should extract transcript slice for completed todo', () => {
      const { extractTranscriptSlice } = require('../supervisor-check.cjs');

      // Mock transcript events
      const events = [
        { type: 'todo_write', timestamp: new Date('2026-01-21T10:00:00Z'), todos: [{ content: 'Task 1', status: 'in_progress' }] },
        { type: 'tool_call', timestamp: new Date('2026-01-21T10:01:00Z'), tool: 'Read', input: { file_path: '/test.ts' } },
        { type: 'tool_result', timestamp: new Date('2026-01-21T10:01:05Z'), content: 'file contents' },
        { type: 'tool_call', timestamp: new Date('2026-01-21T10:02:00Z'), tool: 'Edit', input: { file_path: '/test.ts' } },
        { type: 'tool_result', timestamp: new Date('2026-01-21T10:02:05Z'), content: 'success' },
        { type: 'todo_write', timestamp: new Date('2026-01-21T10:03:00Z'), todos: [{ content: 'Task 1', status: 'completed' }] }
      ];

      const slice = extractTranscriptSlice(events, 'Task 1');

      expect(slice).toContain('Read');
      expect(slice).toContain('Edit');
      expect(slice).toContain('/test.ts');
    });
  });

  describe('WorkerMonitor with supervisor integration', () => {
    test('onTodoUpdate should trigger supervisor check for newly completed todos', async () => {
      const supervisorCheckFn = jest.fn().mockResolvedValue({ passed: true });
      ...
      monitor.setCallbacks({
        onTodoUpdate: (update) => {
          ...
          // This is where supervisor check should be triggered
          if (update.newlyCompleted && update.newlyCompleted.length > 0) {
            for (const todo of update.newlyCompleted) {
              supervisorCheckFn(todo);
            }
          }
        }
      });

      // Simulate todo changes
      monitor.lastTodos = [
      ...
      const update = monitor.detectTodoChanges(newTodos);

      // Verify newly completed is detected
      expect(update.newlyCompleted).toHaveLength(1);
      expect(update.newlyCompleted[0].content).toBe('Task 1');

      // Simulate the callback being called
      if (monitor.callbacks.onTodoUpdate) {
        monitor.callbacks.onTodoUpdate(update);
      }

      // Verify supervisor check was called
      expect(supervisorCheckFn).toHaveBeenCalledWith(
        expect.objectContaining({ content: 'Task 1', status: 'completed' })
      );
    });
  });
});
