...
const SAVE_INTERVAL = 5000; // Save dashboard state every 5 seconds (separate file to avoid manifest conflicts)
...
// Read E2E test progress from verbose config output
function readE2EProgress() {
  const E2E_PROGRESS_PATH = path.join(PROJECT_PATH, '.pipeline', 'e2e-progress.json');
  try {
    if (fs.existsSync(E2E_PROGRESS_PATH)) {
      const content = fs.readFileSync(E2E_PROGRESS_PATH, 'utf8');
    ...
    // Silently ignore - e2e progress is optional
  }
  return null;
}
...
function saveActiveMs() {
  // Write to separate dashboard-state.json to avoid manifest conflicts
  // (Orchestrator writes to manifest.json, dashboard writes here)
  try {
    const state = {
      activeMs: activeMs,
    ...
    // Ignore save errors - timer continues in memory
  }
}
...
function render() {
  ...
  const lines = [];
  const MIN_WIDTH = 80; // Minimum width to prevent negative column calculations
  ...
  // Step Mode and Iteration display
  const stepMode = manifest.stepMode === true || manifest.stepMode === 'true';
  const currentIteration = manifest.currentIteration || 1;
  const iterationCount = manifest.iterations ? manifest.iterations.length : 1;
  const stepModeStr = stepMode ? C.magenta + '⏸ STEP' + C.reset : C.dim + 'AUTO' + C.reset;
  const iterationStr = stepMode ? '  ' + C.dim + '│' + C.reset + '  ' + C.bold + '🔄' + C.reset + ' ' + C.cyan + 'v' + currentIteration + C.reset + (iterationCount > 1 ? C.dim + '/' + iterationCount + C.reset : '') : '';

  let headerLine = '  ' + C.bold + 'Project:' + C.reset + ' ' + projectName.slice(0, 20) + '  ' + C.dim + '│' + C.reset + '  ' + C.bold + 'Mode:' + C.reset + ' ' + mode + '  ' + C.dim + '│' + C.reset + '  ' + stepModeStr + iterationStr + '  ' + C.dim + '│' + C.reset + '  ' + C.bold + '📚' + C.reset + ' ' + onboardingColor + onboardingLevel + C.reset + '  ' + C.dim + '│' + C.reset + '  ' + C.bold + '⏱' + C.reset + ' ' + timerStr;

  // Add checkpoint indicator if in step mode review
  if (manifest.status === 'checkpoint') {
    const checkpointPhase = manifest.currentPhase || '?';
    const checkpointEpic = manifest.phases?.[checkpointPhase]?.currentEpic || null;
    const checkpointLabel = checkpointEpic ? `Phase ${checkpointPhase} Epic ${checkpointEpic}` : `Phase ${checkpointPhase}`;
  ...
  // E2E TESTING section (v9.1) - only show when e2e-progress.json exists and recent
  const e2eProgress = readE2EProgress();
  if (e2eProgress && e2eProgress.status !== 'idle') {
    lines.push(C.cyan + '╠' + '═'.repeat(W-2) + '╣' + C.reset);
    lines.push(line('  ' + C.bold + C.white + 'E2E TESTING' + C.reset + ' ' + C.dim + '(verbose mode)' + C.reset, W));
    lines.push(separator(W));

    // Status line: status + test counts
    const statusIcon = e2eProgress.status === 'running' ? C.yellow + '▶' + C.reset :
                       e2eProgress.status === 'passed' ? C.green + '✓' + C.reset :
                       e2eProgress.status === 'failed' ? C.red + '✗' + C.reset : C.dim + '○' + C.reset;
    const statusColor = e2eProgress.status === 'running' ? C.yellow :
                        e2eProgress.status === 'passed' ? C.green :
                        e2eProgress.status === 'failed' ? C.red : C.dim;
    const passedStr = C.green + e2eProgress.testsPassed + C.reset;
    const failedStr = e2eProgress.testsFailed > 0 ? C.red + e2eProgress.testsFailed + C.reset : C.dim + '0' + C.reset;
    lines.push(line('  ' + statusIcon + ' ' + statusColor + e2eProgress.status.toUpperCase() + C.reset + '    ' + C.dim + 'Passed:' + C.reset + ' ' + passedStr + '  ' + C.dim + 'Failed:' + C.reset + ' ' + failedStr + '  ' + C.dim + 'Total:' + C.reset + ' ' + e2eProgress.testsRun, W));

    // Current test/suite
    if (e2eProgress.currentSuite || e2eProgress.currentTest) {
      const suiteStr = e2eProgress.currentSuite ? C.cyan + truncate(e2eProgress.currentSuite, 30) + C.reset : '';
      const testStr = e2eProgress.currentTest ? C.white + truncate(e2eProgress.currentTest, 40) + C.reset : '';
      lines.push(line('  ' + C.dim + 'Suite:' + C.reset + ' ' + suiteStr + '  ' + C.dim + 'Test:' + C.reset + ' ' + testStr, W));
    }

    // Current step (what WebDriver is doing)
    if (e2eProgress.currentStep && e2eProgress.status === 'running') {
      lines.push(line('  ' + C.dim + 'Step:' + C.reset + ' ' + C.yellow + truncate(e2eProgress.currentStep, W - 12) + C.reset, W));
    }

    // Recent actions (last 3)
    if (e2eProgress.recentActions && e2eProgress.recentActions.length > 0) {
      const recentActions = e2eProgress.recentActions.slice(-3);
      lines.push(line('  ' + C.dim + 'Recent:' + C.reset, W));
      for (const action of recentActions) {
        const selectorPart = action.selector ? ' ' + C.cyan + truncate(action.selector, 30) + C.reset : '';
        lines.push(line('    ' + C.dim + '→' + C.reset + ' ' + action.type + selectorPart, W));
      }
    }

    // Last error (if failed)
    if (e2eProgress.lastError) {
      lines.push(line('  ' + C.red + C.bold + 'LAST ERROR:' + C.reset, W));
      lines.push(line('    ' + C.dim + 'Test:' + C.reset + ' ' + truncate(e2eProgress.lastError.test, W - 12), W));
      lines.push(line('    ' + C.dim + 'Msg:' + C.reset + ' ' + C.red + truncate(e2eProgress.lastError.message, W - 11) + C.reset, W));
      if (e2eProgress.lastError.selector) {
        lines.push(line('    ' + C.dim + 'Selector:' + C.reset + ' ' + C.cyan + e2eProgress.lastError.selector + C.reset, W));
      }
      if (e2eProgress.lastError.expected) {
        lines.push(line('    ' + C.dim + 'Expected:' + C.reset + ' ' + C.green + truncate(e2eProgress.lastError.expected, W - 15) + C.reset, W));
      }
      if (e2eProgress.lastError.actual) {
        lines.push(line('    ' + C.dim + 'Actual:' + C.reset + ' ' + C.red + truncate(e2eProgress.lastError.actual, W - 13) + C.reset, W));
      }
      if (e2eProgress.lastError.screenshot) {
        lines.push(line('    ' + C.dim + 'Screenshot:' + C.reset + ' ' + e2eProgress.lastError.screenshot, W));
      }
    }
  ...
  // Iteration history (shown in step mode with multiple iterations)
  const stepModeActive = manifest.stepMode === true || manifest.stepMode === 'true';
  const iterations = manifest.iterations || [];
  if (stepModeActive && iterations.length > 1) {
    lines.push(line('', W));
    lines.push(C.cyan + '╠' + '═'.repeat(W-2) + '╣' + C.reset);
    lines.push(line('  ' + C.bold + C.magenta + 'ITERATIONS' + C.reset + ' ' + C.dim + '(' + iterations.length + ' total)' + C.reset, W));

    // Show last 3 iterations (or all if fewer)
    const recentIterations = iterations.slice(-3);
    for (const iter of recentIterations) {
      const isActive = iter.id === (manifest.currentIteration || 1);
      const indicator = isActive ? C.cyan + '▶' + C.reset : C.dim + ' ' + C.reset;
      const versionStr = C.bold + 'v' + iter.id + C.reset;
      const branchStr = iter.branch ? C.dim + ' (' + iter.branch + ')' + C.reset : '';
      const statusStr = iter.endReason ? C.yellow + iter.endReason + C.reset : (isActive ? C.green + 'active' + C.reset : C.dim + '--' + C.reset);
      const costStr = iter.cost ? C.green + '$' + iter.cost.toFixed(2) + C.reset : C.dim + '$--' + C.reset;

      lines.push(line('    ' + indicator + ' ' + versionStr + branchStr + '  ' + statusStr + '  ' + costStr, W));
    }

    if (iterations.length > 3) {
      lines.push(line('    ' + C.dim + '... and ' + (iterations.length - 3) + ' earlier' + C.reset, W));
    }
  }
...
// Get current orchestrator PID from manifest (updated on resume) or fall back to command-line arg
function getCurrentOrchestratorPid() {
  const manifest = readManifest();
  return manifest?.orchestratorPid || ORCHESTRATOR_PID;
}
...
function setOrchestratorOutputStyle() {
  const orchestratorPid = getCurrentOrchestratorPid();
  if (!orchestratorPid) return;
  ...
  setTimeout(() => {
    ...
    // Re-read PID in case it changed during the delay
    const currentPid = getCurrentOrchestratorPid();
    if (!currentPid) return;
    console.log('[STARTUP] Now setting orchestrator output style: ' + outputStyle + ' (PID: ' + currentPid + ')');
...
Write-Output "Sending /output-style ${outputStyle} to PID ${currentPid}..."
$r1 = [StyleInjector]::SendText(${currentPid}, "/output-style ${outputStyle}")
...
$r2 = [StyleInjector]::SendEnter(${currentPid})
Write-Output "Enter result: $r2"
`;

    try {
      fs.writeFileSync(scriptPath, psScript);
      const proc = spawn('powershell.exe', ['-ExecutionPolicy', 'Bypass', '-WindowStyle', 'Hidden', '-File', scriptPath], {
...
function triggerContextRefresh() {
  ...
  // Get current orchestrator PID from manifest
  const orchestratorPid = getCurrentOrchestratorPid();
  if (!orchestratorPid) {
    console.log('[REFRESH] No orchestrator PID available');
    return;
  }
...
# Step 1: Send /clear
Write-Output "Sending /clear to PID ${orchestratorPid}..."
$r1 = [RefreshInjector]::SendText(${orchestratorPid}, "/clear")
Start-Sleep -Milliseconds 800
$r1e = [RefreshInjector]::SendEnter(${orchestratorPid})
...
# Step 3: Send /orchestrator-desktop-v9.0 AUTO-RESUME-NO-QUESTION
Write-Output "Sending /orchestrator-desktop-v9.0 AUTO-RESUME-NO-QUESTION..."
$r3 = [RefreshInjector]::SendText(${orchestratorPid}, "/orchestrator-desktop-v9.0 AUTO-RESUME-NO-QUESTION")
Start-Sleep -Milliseconds 800
$r3e = [RefreshInjector]::SendEnter(${orchestratorPid})
...
function sendHeartbeat(manual = false) {
  ...
  // Get orchestrator PID from manifest (updated on resume) or fall back to command-line arg
  const orchestratorPid = manifest?.orchestratorPid || ORCHESTRATOR_PID;
  if (!orchestratorPid) return;

  // Schedule next heartbeat first (only for automatic heartbeats)
  if (!manual) scheduleNextHeartbeat();

  // Check if we should send (skip if complete, paused, or at checkpoint)
  if (!manifest || manifest.status === 'complete' || manifest.status === 'paused' || manifest.status === 'checkpoint') return;
  // Skip pause check for manual heartbeats - Enter key should always work
  if (!manual && manifest.heartbeat && manifest.heartbeat.enabled === false) return;
  ...
  if (needsRefresh) {
    ...
    console.log('[HEARTBEAT ' + heartbeatCount + '] Triggering /clear + /orchestrator-desktop-v9.0 AUTO-RESUME-NO-QUESTION...');
    triggerContextRefresh();
    return;
  }

  console.log('\n[HEARTBEAT ' + heartbeatCount + '] Pinging orchestrator PID ' + orchestratorPid + ' (from manifest) (' + currentCount + '/' + refreshEvery + ')...');
...
$r1 = [ConsoleInjector]::SendString(${orchestratorPid}, "HEARTBEAT: Read worker console, extract todo progress (X/Y), update manifest.phases[phase].workerProgress", $false)
Start-Sleep -Milliseconds 800
$r2 = [ConsoleInjector]::SendString(${orchestratorPid}, "", $true)
...
function sendEventMessage(message) {
  ...
  console.log('\n[EVENT] Sending to PID ' + orchestratorPid + ': ' + message);
...
$r1 = [EventInjector]::SendString(${orchestratorPid}, "${message}", $false)
Start-Sleep -Milliseconds 800
$r2 = [EventInjector]::SendString(${orchestratorPid}, "", $true)
Write-Output "Text: $r1, Enter: $r2"
`;
...
async function checkStateTransitions() {
  ...
  // Track pipeline status (no message sent - orchestrator detects completion directly)
...
[METRICS] Phase ${phase} started - fetching usage before...`);
      ...
      if (currentStatus === 'complete' && previousStatus === 'running') {
        ...
        // NOTE: Removed automatic "PHASE COMPLETE" message to orchestrator.
        // Orchestrator detects phase completion via heartbeat monitoring of manifest.
      }
...
[METRICS] Epic ${epic.id} started - fetching usage before...`);
      ...
      if (currentStatus === 'complete' && previousStatus === 'running') {
        ...
        // NOTE: Removed automatic "EPIC COMPLETE" message to orchestrator.
        // Orchestrator detects epic completion via heartbeat monitoring of manifest.
      }
...
function setupKeypress() {
  ...
  process.stdin.on('keypress', (str, key) => {
    ...
    // Manual heartbeat: Enter (always works, even when paused)
    if (key.name === 'return') {
      sendHeartbeat(true);
      return;
    }
...
function main() {
  ...
  // Load activeMs from dashboard-state.json (separate from manifest to avoid conflicts)
  try {
    if (fs.existsSync(DASHBOARD_STATE_PATH)) {
      const state = JSON.parse(fs.readFileSync(DASHBOARD_STATE_PATH, 'utf8'));
      if (typeof state.activeMs === 'number') {
        activeMs = state.activeMs;
        console.log('Resuming timer from ' + formatTime(activeMs));
      }
      if (typeof state.heartbeatCount === 'number') {
    ...
    activeMs = 0;
    console.log('Starting timer from 0 (state file error)');
  }

  // Read manifest for other settings
  const manifest = readManifest();
  if (manifest) {
  ...
  // Handle exit - save timer and show cursor
  // Note: We don't use alternate screen buffer anymore (it causes window shrinking on Windows conhost)
  process.on('SIGINT', () => { process.stdout.write('\x1B[?25h'); saveActiveMs(); process.exit(0); });
  process.on('SIGTERM', () => { process.stdout.write('\x1B[?25h'); saveActiveMs(); process.exit(0); });

  // Clear screen and start rendering
  // (alternate screen buffer removed - it was causing window to shrink on Windows)
  clearScreen();
  renderAndTrack();
}
