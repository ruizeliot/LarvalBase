...
// Weekly budget for Max 20x subscription ($200/month = $50/week)
// 100% of 7-day limit = $50, so 1% = $0.50
const WEEKLY_BUDGET_USD = 50;
const DOLLARS_PER_PERCENT = 0.50;
...
async function main() {
  console.log('=== 7-DAY LIMIT CALIBRATION ===\n');
  ...
  console.log('  - Tokens between = exact tokens per 1% of weekly limit\n');
  ...
  const initialPercent = Math.floor(usage.sevenDay);
  console.log('Initial usage: ' + usage.sevenDay + '% (floor: ' + initialPercent + '%)');
  ...
  while (iteration < maxIterations) {
    ...
    console.log('Usage: ' + usage.sevenDay.toFixed(1) + '%, Tokens: ' + currentTokens.toLocaleString());

    // Check if we crossed first boundary
    if (!crossedFirst && usage.sevenDay >= firstBoundary) {
      crossedFirst = true;
      startTokens = currentTokens;
      startPercent = usage.sevenDay;
    ...
    if (crossedFirst && usage.sevenDay >= secondBoundary) {
      const endTokens = currentTokens;
      const endPercent = usage.sevenDay;
      ...
      console.log('=== CALIBRATION RESULTS (7-DAY) ===');
      ...
      console.log('\n*** TOKENS PER 1% (7-DAY): ' + Math.round(tokensPerPercent).toLocaleString() + ' ***');
      console.log('*** 1% = $' + DOLLARS_PER_PERCENT.toFixed(2) + ' of weekly budget ***');

      // Save result
      const result = {
        calibrationDate: new Date().toISOString(),
        calibrationType: '7-day',
        tokensPerPercent7Day: Math.round(tokensPerPercent),
        weeklyBudgetUSD: WEEKLY_BUDGET_USD,
        dollarsPerPercent: DOLLARS_PER_PERCENT,
        tokensPerPercent: Math.round(tokensPerPercent),  // Legacy
      ...
      // Also save to lib/calibration.json for orchestrator
      fs.writeFileSync(
        path.join(__dirname, 'calibration.json'),
        JSON.stringify(result, null, 2)
      );
      console.log('Saved to test-calibration/calibration-result.json');
      console.log('Saved to lib/calibration.json');
      return;
    }
  }

  console.log('\nMax iterations reached without crossing both boundaries.');
  console.log('Current usage: ' + usage.sevenDay + '%');
