...
describe('Dashboard Version Selection', () => {
  test('DASHBOARD_SCRIPTS has all versions mapped', () => {
    expect(process.DASHBOARD_SCRIPTS).toBeDefined();
    expect(process.DASHBOARD_SCRIPTS['v9']).toBe('dashboard-v2.cjs');
    expect(process.DASHBOARD_SCRIPTS['v10']).toBe('dashboard-v3.cjs');
    expect(process.DASHBOARD_SCRIPTS['v11']).toBe('dashboard-runner-v11.cjs');
  });

  test('getDashboardScript returns v11 by default', () => {
    const script = process.getDashboardScript();
    expect(script).toContain('dashboard-runner-v11.cjs');
  });

  test('getDashboardScript returns correct script for each version', () => {
    expect(process.getDashboardScript('v9')).toContain('dashboard-v2.cjs');
    expect(process.getDashboardScript('v10')).toContain('dashboard-v3.cjs');
    expect(process.getDashboardScript('v11')).toContain('dashboard-runner-v11.cjs');
  });

  test('getDashboardScript auto-detects v11 from manifest', () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dashboard-version-test-'));
    ...
    const script = process.getDashboardScript('auto', tempDir);
    expect(script).toContain('dashboard-runner-v11.cjs');

    fs.rmSync(tempDir, { recursive: true, force: true });
  });

  test('getDashboardScript auto-detects v10 from manifest', () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dashboard-version-test-'));
    const pipelineDir = path.join(tempDir, '.pipeline');
    fs.mkdirSync(pipelineDir, { recursive: true });
    ...
    const script = process.getDashboardScript('auto', tempDir);
    expect(script).toContain('dashboard-v3.cjs');
...
describe('Module Exports', () => {
  ...
  test('exports spawn functions', () => {
    ...
    expect(typeof process.getDashboardScript).toBe('function');
    expect(process.DASHBOARD_SCRIPTS).toBeDefined();
  });
