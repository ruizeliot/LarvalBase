...
function render() {
  ...
  for (const phase of phases) {
    ...
    // Fixed layout: cursor(2) + left section (22 chars) + padding + right section (stats ~52 chars) + 2 right padding
    const phaseNameTrunc = truncate(name, 12).padEnd(12);
    const cursor = isCursor(phase) ? C.cyan + '>' + C.reset + ' ' : '  ';
    const leftPart = cursor + icon + '  ' + phase + '. ' + expandIcon + rowColor + phaseNameTrunc + C.reset;
    const leftWidth = 24; // 2 cursor + 1 + 2 + 1 + 2 + 2 + 12 = 24
    const statsWidth = 52;
    const rightPad = 2;
    ...
    if (isExpanded && hasTodos) {
      ...
      for (let i = 0; i < todos.length; i++) {
        ...
        // Right-aligned: cursor(2) + indent(4) + prefix(2) + expand(2) + name(18) + padding + stats(52) + rightPad(2)
        const todoLeft = todoCursor + '    ' + C.dim + prefix + C.reset + ' ' + todoExpandIcon + todoName;
        const todoLeftWidth = 31; // 2 + 4 + 2 + 1 + 2 + 18 = 29, but add 2 for expand icon
  ...
  if (manifest.epics && manifest.epics.length > 0) {
    ...
    for (const epic of manifest.epics) {
      ...
      // Fixed layout: cursor(2) + left section (24 chars) + padding + right section (stats ~52 chars) + 2 right padding
      const epicNameTrunc = truncate(epic.name, 14).padEnd(14);
      const epicCursor = isCursor(epic.id) ? C.cyan + '>' + C.reset + ' ' : '  ';
      const epicLeftPart = epicCursor + epicIcon + '  ' + epicKey + '. ' + expandIndicator + epicRowColor + epicNameTrunc + C.reset;
      const epicLeftWidth = 26; // 2 cursor + 1 + 2 + 2 + 2 + 1 + 14 = 24 (padded to 26)
      ...
      if (isEpicExpanded && epic.todoBreakdown && epic.todoBreakdown.length > 0) {
        ...
        for (let i = 0; i < epic.todoBreakdown.length; i++) {
          ...
          // Right-aligned: cursor(2) + indent(4) + prefix(2) + expand(2) + name(16) + padding + stats(52) + rightPad(2)
          const epicTodoLeft = epicTodoCursor + '    ' + C.dim + prefix + C.reset + ' ' + todoExpandIcon + todoName;
          const epicTodoLeftWidth = 28; // 2 + 4 + 2 + 2 + 2 + 16
