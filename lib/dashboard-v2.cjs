...
function render() {
  ...
  for (let i = 0; i < LOGO.length; i++) {
    ...
    const version = (i === LOGO.length - 1) ? '  ' + C.dim + 'v7.0' + C.reset : '';
    lines.push(line('  ' + C.cyan + logoLine + C.reset + version, W));
  }
  lines.push(line('', W));

  // Project info and timer on same line
  const timerStr = formatTime(activeMs);
  lines.push(line('  ' + C.bold + 'Project:' + C.reset + ' ' + projectName.slice(0, 25) + '  ' + C.dim + '│' + C.reset + '  ' + C.bold + 'Mode:' + C.reset + ' ' + mode + '  ' + C.dim + '│' + C.reset + '  ' + C.bold + '⏱' + C.reset + ' ' + timerStr, W));
  lines.push(C.cyan + '╠' + '═'.repeat(W-2) + '╣' + C.reset);
  ...
  lines.push(line('', W));
  lines.push(line('  ' + C.bold + C.white + 'PHASES' + C.reset + '  ' + C.dim + phaseHint + C.reset, W));
  ...
  for (const phase of phases) {
    ...
    if (status === 'complete' && phaseData.duration) {
      ...
      stats = C.white + formatTime(phaseData.duration).padStart(12) + C.reset + '  ';
      stats += C.green + formatCost(phaseData.cost).padStart(7) + C.reset + '  ';
      stats += C.magenta + formatTokens(phaseData.tokens).padStart(6) + C.reset;
      ...
      stats = C.yellow + formatTime(elapsed).padStart(12) + C.reset;
    } else {
      stats = C.dim + '--'.padStart(12) + '  ' + '--'.padStart(7) + '  ' + '--'.padStart(6) + C.reset;
    }

    const phaseLine = '  ' + icon + '  ' + phase + '. ' + expandIcon + rowColor + name.padEnd(10) + C.reset + stats;
    ...
    if (isExpanded && hasTodos) {
      ...
      for (let i = 0; i < todos.length; i++) {
        ...
        const prefix = isLast ? '└─' : '├─';
        const todoName = truncate(todo.content, 26);
        const todoDur = formatTime(todo.durationMs).padStart(10);
        const todoCost = formatCost(todo.cost).padStart(7);
        const todoTokens = todo.tokens ? formatTokens(todo.tokens).padStart(6) : '--'.padStart(6);
        lines.push(line('      ' + C.dim + prefix + C.reset + ' ' + todoName.padEnd(26) + C.dim + todoDur + '  ' + C.green + todoCost + C.reset + '  ' + C.magenta + todoTokens + C.reset, W));
      }
    }
    ...
    if (phase === '4' && manifest.epics && manifest.epics.length > 0 && !isExpanded) {
      // Epic section header
      lines.push(line('', W));
      lines.push(line('  ' + C.bold + C.white + 'EPICS' + C.reset + '  ' + C.dim + '[A-I]' + C.reset, W));
      lines.push(separator(W));
      ...
      for (const epic of manifest.epics) {
        ...
        let epicStats = '';
        if (epic.duration) {
          epicStats += C.white + formatTime(epic.duration).padStart(12) + C.reset + '  ';
          epicStats += C.green + formatCost(epic.cost).padStart(7) + C.reset + '  ';
          epicStats += C.magenta + formatTokens(epic.tokens).padStart(6) + C.reset;
        } else {
          epicStats += C.dim + '--'.padStart(12) + '  ' + '--'.padStart(7) + '  ' + '--'.padStart(6) + C.reset;
        }
        if (epic.todoBreakdown) epicStats += C.dim + '  (' + epic.todoBreakdown.length + ')' + C.reset;

        const epicName = truncate(epic.name, 22);
        const epicLine = '  ' + epicIcon + '  ' + C.yellow + '[' + epicKey + ']' + C.reset + ' ' + expandIndicator + ' ' + epicRowColor + epicName.padEnd(22) + C.reset + epicStats;
        ...
        if (isEpicExpanded && epic.todoBreakdown && epic.todoBreakdown.length > 0) {
          for (let i = 0; i < epic.todoBreakdown.length; i++) {
            ...
            const prefix = isLast ? '└─' : '├─';
            const todoName = truncate(todo.content || '', 24);
            const todoDur = todo.durationMs ? formatTime(todo.durationMs).padStart(10) : '--'.padStart(10);
            const todoCost = todo.cost ? formatCost(todo.cost).padStart(7) : '--'.padStart(7);
            const todoTokens = todo.tokens ? formatTokens(todo.tokens).padStart(6) : '--'.padStart(6);
            lines.push(line('        ' + C.dim + prefix + C.reset + ' ' + todoName.padEnd(24) + C.dim + todoDur + '  ' + C.green + todoCost + C.reset + '  ' + C.magenta + todoTokens + C.reset, W));
          }
        }
      }
    }
  ...
  if (!workerProgress || workerProgress.total === 0) {
    ...
    const countStr = workerProgress.completed + '/' + workerProgress.total;
    lines.push(line('  [' + bar + '] ' + C.bold + pctStr + C.reset + '  ' + C.dim + countStr + C.reset, W));
    if (workerProgress.currentTask) {
      const task = truncate(workerProgress.currentTask, W-10);
      lines.push(line('  ' + C.cyan + '→ ' + C.reset + C.dim + task + C.reset, W));
    }
  }
  ...
  lines.push(line('  ' + C.bold + C.white + 'TOTALS' + C.reset + '    ' + C.green + formatCost(manifest.totalCost || 0) + C.reset + '   ' + C.magenta + formatTokens(totalTokens) + ' tokens' + C.reset, W));


  // Heartbeat status - append to totals line
  if (manifest.status !== 'complete') {
    ...
    if (ORCHESTRATOR_PID) {
      if (heartbeatEnabled && nextHeartbeatTime) {
        ...
        heartbeatInfo = '   ' + C.dim + '│' + C.reset + '   ' + C.bold + 'HEARTBEAT' + C.reset + ' ' + countdownStr + '  ' + C.dim + currentCount + '/' + refreshEvery + C.reset;
      } else if (!heartbeatEnabled) {
        heartbeatInfo = '   ' + C.dim + '│' + C.reset + '   ' + C.bold + 'HEARTBEAT' + C.reset + ' ' + C.yellow + 'PAUSED' + C.reset;
      }
    }
    // Append heartbeat to the totals line
    if (heartbeatInfo && lines.length > 0) {
      const lastIdx = lines.length - 1;
      const lastLine = lines[lastIdx];
      // Remove the right border, add heartbeat, re-add border
      const newContent = lastLine.replace(new RegExp(C.cyan.replace(/\[/g, '\\[') + '║' + C.reset.replace(/\[/g, '\\[') + '$'), '') + heartbeatInfo;
      lines[lastIdx] = line(newContent.replace(/^.*?║/, ''), W);
    }
  }
  ...
  lines.push(line('', W));
  lines.push(line('  ' + C.dim + 'Pricing: Opus=' + C.green + '$5' + C.dim + '/' + C.green + '$25' + C.dim + '/MTok  Sonnet=' + C.green + '$3' + C.dim + '/' + C.green + '$15' + C.dim + '  Haiku=' + C.green + '$1' + C.dim + '/' + C.green + '$5' + C.dim + '  (Cache: 0.1x read)' + C.reset, W));

  // Footer with hotkey hints
  lines.push(C.cyan + '╠' + '═'.repeat(W-2) + '╣' + C.reset);
  lines.push(line('  ' + C.yellow + '[1-5]' + C.reset + ' Phases    ' + C.yellow + '[A-I]' + C.reset + ' Epics    ' + C.yellow + '[Space]' + C.reset + ' Heartbeat    ' + C.yellow + '[Q]' + C.reset + ' Quit', W));
  lines.push(C.cyan + '╚' + '═'.repeat(W-2) + '╝' + C.reset);
