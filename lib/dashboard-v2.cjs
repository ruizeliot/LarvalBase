...
function render() {
  ...
  for (const phase of phases) {
    ...
    if (isExpanded && hasTodos) {
      ...
      if ((phaseUsageDelta === undefined || phaseUsageDelta === null || phaseUsageDelta === 0) && phaseData.tokens && tokensPerPercent) {
        phaseUsageDelta = phaseData.tokens / tokensPerPercent;
      }
  ...
  if (manifest.epics && manifest.epics.length > 0) {
    ...
    for (const epic of manifest.epics) {
      ...
      if (isEpicExpanded && epic.todoBreakdown && epic.todoBreakdown.length > 0) {
        ...
        if ((epicUsageDelta === undefined || epicUsageDelta === null || epicUsageDelta === 0) && epic.tokens && tokensPerPercent) {
          epicUsageDelta = epic.tokens / tokensPerPercent;
        }
...
function analyzeAndStoreOrchestratorMetrics() {
  ...
  const transcriptInfo = findMostRecentTranscript();
  if (!transcriptInfo) {
  ...
  // Estimate usageDelta from tokens using calibration
  const estimatedPercent = manifest.orchestrator.totalTokens / tokensPerPercent;
  manifest.orchestrator.totalUsageDelta = estimatedPercent;
...
function main() {
  ...
  if (manifest) {
    ...
    let recalcCount = 0;
    if (manifest.phases && tokensPerPercent) {
      for (const [phase, data] of Object.entries(manifest.phases)) {
        if (data.status === 'complete' && data.tokens && data.tokens > 0) {
          const oldDelta = data.usageDelta;
          const newDelta = Math.round((data.tokens / tokensPerPercent) * 100) / 100;
    ...
    if (manifest.epics && tokensPerPercent) {
      for (const epic of manifest.epics) {
        if (epic.status === 'complete' && epic.tokens && epic.tokens > 0) {
          const oldDelta = epic.usageDelta;
          const newDelta = Math.round((epic.tokens / tokensPerPercent) * 100) / 100;
          if (oldDelta !== newDelta) {
