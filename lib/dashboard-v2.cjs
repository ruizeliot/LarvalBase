...
function render() {
  ...
  lines.push(line('', W));
  // Column headers: regular (yellow), cached (cyan), total (green)
  const colHeader = C.dim + '                              ' + C.yellow + 'regular' + C.reset + '         ' + C.cyan + 'cached' + C.reset + '          ' + C.green + 'total' + C.reset;
  lines.push(line('  ' + C.bold + C.white + 'PHASES' + C.reset + '  ' + C.dim + phaseHint + C.reset + colHeader, W));
  ...
  for (const phase of phases) {
    ...
    if (status === 'complete' && phaseData.duration) {
      ...
      stats = C.yellow + formatTime(elapsed).padStart(10) + C.reset;
    } else {
    ...
    // Expanded todo breakdown with token/cost breakdown
    if (isExpanded && hasTodos) {
      ...
      for (let i = 0; i < todos.length; i++) {
        ...
        const prefix = isLast ? '└─' : '├─';
        const todoName = truncate(todo.content, 22);
        ...
        todoStats += C.yellow + todoRegTok.padStart(5) + ' ' + todoRegCost.padStart(5) + C.reset + ' ';
        todoStats += C.cyan + todoCacheTok.padStart(5) + ' ' + todoCacheCost.padStart(5) + C.reset + ' ';
        todoStats += C.green + todoTotTok.padStart(5) + ' ' + todoTotCost.padStart(5) + C.reset;

        lines.push(line('      ' + C.dim + prefix + C.reset + ' ' + todoName.padEnd(22) + todoStats, W));
      }
    }
  ...
  if (manifest.epics && manifest.epics.length > 0) {
    lines.push(line('', W));
    const epicColHeader = C.dim + '                            ' + C.yellow + 'regular' + C.reset + '         ' + C.cyan + 'cached' + C.reset + '          ' + C.green + 'total' + C.reset;
    lines.push(line('  ' + C.bold + C.white + 'EPICS' + C.reset + '  ' + C.dim + '[A-I]' + C.reset + epicColHeader, W));
    lines.push(separator(W));
    ...
    for (const epic of manifest.epics) {
      ...
      const epicName = truncate(epic.name, 18);
      const epicLine = '  ' + epicIcon + '  ' + C.yellow + '[' + epicKey + ']' + C.reset + ' ' + expandIndicator + ' ' + epicRowColor + epicName.padEnd(18) + C.reset + epicStats;
      lines.push(line(epicLine, W));

      // Expanded epic details with token/cost breakdown
      if (isEpicExpanded && epic.todoBreakdown && epic.todoBreakdown.length > 0) {
        for (let i = 0; i < epic.todoBreakdown.length; i++) {
          ...
          const prefix = isLast ? '└─' : '├─';
          const todoName = truncate(todo.content || '', 20);
          const todoDur = todo.durationMs ? formatTime(todo.durationMs).padStart(8) : '--'.padStart(8);
          // Regular (yellow)
          const todoRegTok = todo.regularTokens ? formatTokens(todo.regularTokens) : '--';
          const todoRegCost = todo.regularCost !== undefined ? formatCost(todo.regularCost) : '$--';
          // Cached (cyan)
          ...
          lines.push(line('        ' + C.dim + prefix + C.reset + ' ' + todoName.padEnd(20) + todoStats, W));
        }
      }
    }
  }
  ...
  let totalsLine = '  ' + C.bold + C.white + 'TOTALS' + C.reset + '                    ';
  totalsLine += C.yellow + formatTokens(totalRegularTokens).padStart(6) + ' ' + formatCost(totalRegularCost).padStart(6) + C.reset + ' ';
  totalsLine += C.cyan + formatTokens(totalCachedTokens).padStart(6) + ' ' + formatCost(totalCachedCost).padStart(6) + C.reset + ' ';
  totalsLine += C.green + formatTokens(totalTokens).padStart(6) + ' ' + formatCost(manifest.totalCost || 0).padStart(6) + C.reset;
  lines.push(line(totalsLine, W));
  ...
  lines.push(line('', W));
  lines.push(line('  ' + C.dim + 'Colors: ' + C.yellow + 'regular' + C.dim + ' | ' + C.cyan + 'cached' + C.dim + ' | ' + C.green + 'total' + C.dim + '    Opus=' + C.reset + '$5/$25' + C.dim + '/MTok  Sonnet=' + C.reset + '$3/$15' + C.dim + '  Haiku=' + C.reset + '$1/$5' + C.dim + '  (Cache: 0.1x)' + C.reset, W));
...
function checkStateTransitions() {
  ...
  if (manifest.phases) {
    for (const [phase, data] of Object.entries(manifest.phases)) {
      ...
      if (currentStatus === 'complete' && previousStatus === 'running') {
        sendEventMessage('PHASE COMPLETE: Analyze session, spawn next phase worker.');
      }
  ...
  if (manifest.epics && manifest.epics.length > 0) {
    for (const epic of manifest.epics) {
      ...
      if (currentStatus === 'complete' && previousStatus === 'running') {
        sendEventMessage('EPIC COMPLETE: Analyze session, spawn next epic worker.');
      }
