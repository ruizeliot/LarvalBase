...
async function runSelectedAnalysis() {
  ...
  if (item.type === 'phase') {
    ...
    if (!data.sessionId) {
      console.log(`\n[ANALYZE] Phase ${phase} has no sessionId`);
      return;
    }
    ...
    if (transcriptPath) {
      ...
      if (metrics) {
        ...
        console.log(`[ANALYZE] Phase ${phase}: ${metrics.tokens} tokens, $${metrics.cost}`);
        analysisRan = true;
      }
    } else {
      console.log(`[ANALYZE] Phase ${phase}: transcript not found`);
    }
    ...
    if (transcriptPath) {
      ...
      if (metrics) {
        ...
        console.log(`[ANALYZE] Epic ${epicId}: ${metrics.tokens} tokens, $${metrics.cost}`);
        analysisRan = true;
      }
    } else {
      console.log(`[ANALYZE] Epic ${epicId}: transcript not found`);
    }
