...
// Pad string to target visible width (accounts for ANSI codes)
...
function render() {
  ...
  const centerIn = (s, w) => { const pad = Math.floor((w - visibleLength(s)) / 2); return ''.padEnd(pad) + s + ''.padEnd(w - pad - visibleLength(s)); };
  ...
  for (const phase of phases) {
    ...
    const noSessionWidth = noSessionWarning ? 2 : 0; // space + icon (⚠ renders as 2 cols)
    // Truncate name to fit: prefix is cursor(2) + icon(1) + spaces(2) + phase#(3) + expand(2) + warning = 10 + warning
    const maxNameLen = Math.max(5, nameColWidth - 10 - noSessionWidth);
    const displayName = truncate(name, maxNameLen);
    const leftPart = cursor + icon + '  ' + phase + '. ' + expandIcon + rowColor + displayName + C.reset + noSessionWarning;
    // Pad name to fill nameColWidth (calculated from W - data columns - separators)
    const leftVisible = 2 + 1 + 2 + 3 + 2 + displayName.length + noSessionWidth; // cursor + icon + spaces + phase# + expand + name + warning
    ...
    if (isExpanded && hasTodos) {
      ...
      if (analysis && analysis.summary) {
        ...
        lines.push(line(indent + C.dim + '│' + C.reset + padToWidth(healthLine, boxWidth - 2) + C.dim + '│' + C.reset, W));
        ...
        if (analysis.whatWentWell && analysis.whatWentWell.length > 0) {
          ...
          for (const item of analysis.whatWentWell.slice(0, 3)) {
            ...
            lines.push(line(indent + C.dim + '│' + C.reset + padToWidth(itemText, boxWidth - 2) + C.dim + '│' + C.reset, W));
          }
        }
        ...
        if (analysis.issuesFound && analysis.issuesFound.length > 0) {
          ...
          for (const issue of analysis.issuesFound.slice(0, 3)) {
            ...
            lines.push(line(indent + C.dim + '│' + C.reset + padToWidth(issueText, boxWidth - 2) + C.dim + '│' + C.reset, W));
          }
        }
        ...
        if (analysis.recommendations && analysis.recommendations.length > 0) {
          ...
          for (const rec of analysis.recommendations.slice(0, 2)) {
            ...
            lines.push(line(indent + C.dim + '│' + C.reset + padToWidth(recText, boxWidth - 2) + C.dim + '│' + C.reset, W));
          }
        }
      ...
      for (let i = 0; i < todos.length; i++) {
        ...
        if (isTodoExpanded && todoAnalysis) {
          ...
          if (todoAnalysis.health === 'clean') {
            ...
            const rootCauseContent = ' ' + healthIcon + ' ROOT CAUSE: ' + rootCauseColor + rootCause + C.reset;
            lines.push(line(boxIndent + C.dim + '│' + C.reset + padToWidth(rootCauseContent, boxWidth - 2) + C.dim + '│' + C.reset, W));

            // Error message if available
            if (todoAnalysis.errorMessage) {
              const errText = ` Error: ${todoAnalysis.errorMessage}`.slice(0, boxWidth - 4);
              lines.push(line(boxIndent + C.dim + '│' + C.reset + padToWidth(C.red + errText + C.reset, boxWidth - 2) + C.dim + '│' + C.reset, W));
            }
            ...
            if (todoAnalysis.retrySequence && todoAnalysis.retrySequence.length > 0) {
              ...
              for (const step of todoAnalysis.retrySequence.slice(0, 4)) {
                ...
                const stepContent = stepText + ' ' + resultIcon + C.reset + ' ' + behaviorIcon + C.reset;
                  lines.push(line(boxIndent + C.dim + '│' + C.reset + padToWidth(stepContent, boxWidth - 2) + C.dim + '│' + C.reset, W));
              }
            }
            ...
            if (todoAnalysis.recommendation) {
              const recContent = C.cyan + ' 💡 ' + todoAnalysis.recommendation.slice(0, boxWidth - 6) + C.reset;
              lines.push(line(boxIndent + C.dim + '│' + C.reset + padToWidth(recContent, boxWidth - 2) + C.dim + '│' + C.reset, W));
            }
  ...
  if (manifest.epics && manifest.epics.length > 0) {
    ...
    for (const epic of manifest.epics) {
      ...
      const epicNoSessionWidth = epicNoSessionWarning ? 2 : 0; // space + icon (⚠ renders as 2 cols)
      // Truncate epic name to fit: prefix is cursor(2) + icon(1) + spaces(2) + key#(3) + expand(1) + space(1) + warning = 10 + warning
      const maxEpicNameLen = Math.max(5, nameColWidth - 10 - epicNoSessionWidth);
      const displayEpicName = truncate(epic.name, maxEpicNameLen);
      const epicLeftPart = epicCursor + epicIcon + '  ' + epicKey + '. ' + expandIndicator + ' ' + epicRowColor + displayEpicName + C.reset + epicNoSessionWarning;
      const epicLeftVisible = 2 + 1 + 2 + 3 + 1 + 1 + displayEpicName.length + epicNoSessionWidth; // cursor + icon + spaces + key# + expand + space + name + warning
          ...
          if (isTodoExpanded && epicTodoAnalysis) {
            ...
            lines.push(line(boxIndent + C.dim + '│' + C.reset + padToWidth(rootCauseContent, boxWidth - 2) + C.dim + '│' + C.reset, W));
              ...
              if (epicTodoAnalysis.recommendation) {
                const epicRecContent = C.cyan + ' 💡 ' + epicTodoAnalysis.recommendation.slice(0, boxWidth - 6) + C.reset;
                lines.push(line(boxIndent + C.dim + '│' + C.reset + padToWidth(epicRecContent, boxWidth - 2) + C.dim + '│' + C.reset, W));
              }
