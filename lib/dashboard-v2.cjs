...
function getSubCost(usageDelta) {
  ...
  return (usageDelta / 100) * WEEKLY_SUB_BUDGET;
}

// Get display cost based on current pricing mode
function getDisplayCost(apiCost, usageDelta) {
  if (pricingMode === 'sub') {
    const subCost = getSubCost(usageDelta);
    return subCost !== null ? subCost : apiCost;
  }
  return apiCost;
}
...
function render() {
  ...
  for (const phase of phases) {
    ...
    if (status === 'complete' && phaseData.duration) {
      ...
      const regPct = (usageDelta && totalCost > 0) ? usageDelta * ((phaseData.regularCost || 0) / totalCost) : null;
      const cachePct = (usageDelta && totalCost > 0) ? usageDelta * ((phaseData.cachedCost || 0) / totalCost) : null;
      ...
      const regCost = phaseData.regularCost !== undefined ? formatCost(pricingMode === 'sub' ? getSubCost(regPct) || phaseData.regularCost : phaseData.regularCost) : '$--';
      ...
      const cacheCost = phaseData.cachedCost !== undefined ? formatCost(pricingMode === 'sub' ? getSubCost(cachePct) || phaseData.cachedCost : phaseData.cachedCost) : '$--';
    ...
    if (isExpanded && hasTodos) {
      ...
      const phaseUsageDelta = phaseData.usageDelta;
      ...
      for (let i = 0; i < todos.length; i++) {
        ...
        const todoUsagePct = phaseUsageDelta ? phaseUsageDelta * todoRatio : null;

        // Regular: proportional percentage in SUB mode
        const todoRegRatio = phaseTotalCost > 0 ? (todo.regularCost || 0) / phaseTotalCost : 0;
        const todoRegPct = phaseUsageDelta ? phaseUsageDelta * todoRegRatio : null;
        ...
        const todoCachePct = phaseUsageDelta ? phaseUsageDelta * todoCacheRatio : null;
  ...
  if (manifest.epics && manifest.epics.length > 0) {
    ...
    for (const epic of manifest.epics) {
      ...
      if (epic.duration) {
        ...
        const eRegPct = (epicUsageDelta && epicTotalCost > 0) ? epicUsageDelta * ((epic.regularCost || 0) / epicTotalCost) : null;
        const eCachePct = (epicUsageDelta && epicTotalCost > 0) ? epicUsageDelta * ((epic.cachedCost || 0) / epicTotalCost) : null;

        // Regular: tokens or % (yellow)
        const eRegVal = pricingMode === 'sub' && eRegPct !== null ? formatPercent(eRegPct) : (epic.regularTokens ? formatTokens(epic.regularTokens) : '--');
        const eRegCost = epic.regularCost !== undefined ? formatCost(pricingMode === 'sub' ? getSubCost(eRegPct) || epic.regularCost : epic.regularCost) : '$--';
        ...
        const eCacheCost = epic.cachedCost !== undefined ? formatCost(pricingMode === 'sub' ? getSubCost(eCachePct) || epic.cachedCost : epic.cachedCost) : '$--';
      ...
      if (isEpicExpanded && epic.todoBreakdown && epic.todoBreakdown.length > 0) {
        ...
        for (let i = 0; i < epic.todoBreakdown.length; i++) {
          ...
          const todoUsagePct = epicUsageDelta ? epicUsageDelta * todoRatio : null;

          // Regular: proportional percentage in SUB mode
          const todoRegRatio = epicTotalCost > 0 ? (todo.regularCost || 0) / epicTotalCost : 0;
          const todoRegPct = epicUsageDelta ? epicUsageDelta * todoRegRatio : null;
          const todoRegVal = pricingMode === 'sub' && todoRegPct !== null ? formatPercent(todoRegPct) : (todo.regularTokens ? formatTokens(todo.regularTokens) : '--');
          const todoRegCost = todo.regularCost !== undefined ? formatCost(todo.regularCost) : '$--';

          // Cached: proportional percentage in SUB mode
          const todoCacheRatio = epicTotalCost > 0 ? (todo.cachedCost || 0) / epicTotalCost : 0;
          const todoCachePct = epicUsageDelta ? epicUsageDelta * todoCacheRatio : null;
          const todoCacheVal = pricingMode === 'sub' && todoCachePct !== null ? formatPercent(todoCachePct) : (todo.cachedTokens ? formatTokens(todo.cachedTokens) : '--');
          const todoCacheCost = todo.cachedCost !== undefined ? formatCost(todo.cachedCost) : '$--';
  ...
  // Calculate total display cost based on pricing mode
  const totalApiCost = manifest.totalCost || (totalRegularCost + totalCachedCost);
  const totalSubCost = getSubCost(totalUsageDelta);
  const totalDisplayCost = pricingMode === 'sub' && totalSubCost !== null ? totalSubCost : totalApiCost;

  // TOTALS line with breakdown: regular (yellow), cached (cyan), total (green)
  // In SUB mode, show percentages instead of tokens
  ...
  // Calculate proportional percentages for regular and cached
  const totalRegPct = (totalUsageDelta && totalDisplayCost > 0) ? totalUsageDelta * (totalRegularCost / (totalRegularCost + totalCachedCost)) : null;
  const totalCachePct = (totalUsageDelta && totalDisplayCost > 0) ? totalUsageDelta * (totalCachedCost / (totalRegularCost + totalCachedCost)) : null;

  // Regular: tokens or % (yellow)
  const totalRegVal = pricingMode === 'sub' && totalRegPct !== null ? formatPercent(totalRegPct) : formatTokens(totalRegularTokens);
  const totalRegCostVal = formatCost(pricingMode === 'sub' ? getSubCost(totalRegPct) || totalRegularCost : totalRegularCost);
  totalsStats += C.yellow + totalRegVal.padStart(6) + ' ' + totalRegCostVal.padStart(6) + C.reset + ' ';

  // Cached: tokens or % (cyan)
  const totalCacheVal = pricingMode === 'sub' && totalCachePct !== null ? formatPercent(totalCachePct) : formatTokens(totalCachedTokens);
  const totalCacheCostVal = formatCost(pricingMode === 'sub' ? getSubCost(totalCachePct) || totalCachedCost : totalCachedCost);
  totalsStats += C.cyan + totalCacheVal.padStart(6) + ' ' + totalCacheCostVal.padStart(6) + C.reset + ' ';

  // Total: tokens or % (green)
  const totalTotVal = pricingMode === 'sub' && totalUsageDelta > 0 ? formatPercent(totalUsageDelta) : formatTokens(totalTokens);
  totalsStats += C.green + totalTotVal.padStart(6) + ' ' + formatCost(totalDisplayCost).padStart(6) + C.reset;
  ...
  if (subscriptionUsage) {
    ...
    lines.push(line('', W));
    lines.push(line('  ' + C.magenta + 'SUBSCRIPTION' + C.reset + '  5hr: ' + fiveHrColor + fiveHr.padStart(4) + C.reset + '  7-day: ' + sevenDayColor + sevenDay.padStart(4) + C.reset, W));
  }
