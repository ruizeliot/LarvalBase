...
// Format data column with dim separators between tok/pct/cost
// SUB mode: tok│%│$ (visual width 18), API mode: tok│$ (visual width 14)
// Returns string with embedded color codes, properly padded to colWidth
function formatDataCol(tok, pct, cost, color, isSubMode, colWidth = 18, suffix = '') {
  let content, visWidth;
  if (isSubMode) {
    // SUB: 5 + 1 + 5 + 1 + 6 = 18 chars visual
    content = tok.padStart(5) + DIM_SEP + color + pct.padStart(5) + DIM_SEP + color + cost.padStart(6);
    visWidth = 18;
  } else {
    // API: 6 + 1 + 6 = 13 chars visual
    content = tok.padStart(6) + DIM_SEP + color + cost.padStart(6);
    visWidth = 13;
  }
  // Add suffix if provided (e.g., "(3)" for todo count)
  const suffixVisWidth = suffix ? suffix.replace(/\x1b\[[0-9;]*m/g, '').length : 0;
  const totalVisWidth = visWidth + suffixVisWidth;
  const padding = Math.max(0, colWidth - totalVisWidth);
  return ' '.repeat(padding) + color + content + suffix + C.reset;
}
...
function render() {
  ...
  // Sub-header row showing column structure: tok│%│$ (SUB) or tok│$ (API)
  const subHeaderFormat = pricingMode === 'sub' ? 'tok│%│$' : 'tok│$';
  const subHeaderRight = COL_SEP + C.dim + ''.padStart(COL.TIME) + C.reset + COL_SEP + C.dim + centerIn(subHeaderFormat, COL.REG) + C.reset + COL_SEP + C.dim + centerIn(subHeaderFormat, COL.CACHE) + C.reset + COL_SEP + C.dim + centerIn(subHeaderFormat, COL.TOTAL) + C.reset;
  ...
  workersStats += formatDataCol(workersRegTok, workersRegPctStr, workersRegCostVal, C.yellow, pricingMode === 'sub', COL.REG) + ' ';
  ...
  workersStats += formatDataCol(workersCacheTok, workersCachePctStr, workersCacheCostVal, C.cyan, pricingMode === 'sub', COL.CACHE) + ' ';
  ...
  workersStats += formatDataCol(workersTotTok, workersTotPctStr, workersTotCostVal, C.green, pricingMode === 'sub', COL.TOTAL);
  ...
  if (hasSupervisorData) {
    ...
    supStats += formatDataCol(supRegTok, supRegPctStr, supRegCostVal, C.yellow, pricingMode === 'sub', COL.REG) + ' ';
    ...
    supStats += formatDataCol(supCacheTok, supCachePctStr, supCacheCostVal, C.cyan, pricingMode === 'sub', COL.CACHE) + ' ';
    ...
    supStats += formatDataCol(supTotTok, supTotPctStr, supTotCostVal, C.magenta, pricingMode === 'sub', COL.TOTAL);
  ...
  if (hasSupervisorData) {
    ...
    grandStats += formatDataCol(grandRegTok, grandRegPctStr, grandRegCostVal, C.yellow, pricingMode === 'sub', COL.REG) + ' ';
    ...
    grandStats += formatDataCol(grandCacheTok, grandCachePctStr, grandCacheCostVal, C.cyan, pricingMode === 'sub', COL.CACHE) + ' ';
    ...
    grandStats += formatDataCol(grandTotTok, grandTotPctStr, grandTotCostVal, C.green, pricingMode === 'sub', COL.TOTAL);
  } else {
    // No supervisor data - grand total = workers total (reuse workers values)
    grandStats += formatDataCol(workersRegTok, workersRegPctStr, workersRegCostVal, C.yellow, pricingMode === 'sub', COL.REG) + ' ';
    grandStats += formatDataCol(workersCacheTok, workersCachePctStr, workersCacheCostVal, C.cyan, pricingMode === 'sub', COL.CACHE) + ' ';
    grandStats += formatDataCol(workersTotTok, workersTotPctStr, workersTotCostVal, C.green, pricingMode === 'sub', COL.TOTAL);
  }
