...
function render() {
  ...
  // Dynamic cost color based on pricing mode
  const costColor = pricingMode === 'api' ? C.green : C.magenta;
  ...
  const headerRight = C.dim + '   time   ' + C.reset + ' ' + C.yellow + centerIn13('regular') + C.reset + ' ' + C.cyan + centerIn13('cached') + C.reset + ' ' + costColor + centerIn13(pricingMode === 'api' ? 'total' : 'sub $') + C.reset;
  ...
  for (const phase of phases) {
    ...
    if (status === 'complete' && phaseData.duration) {
      ...
      // Total tokens + cost (color based on pricing mode)
      ...
      stats += costColor + totTok.padStart(6) + ' ' + totCost.padStart(6) + C.reset;
    ...
    if (isExpanded && hasTodos) {
      ...
      // Calculate total API cost from todos for proportional SUB cost distribution
      ...
      for (let i = 0; i < todos.length; i++) {
        ...
        // Total (color based on pricing mode) - proportional SUB cost for todos
        ...
        todoStats += costColor + todoTotTok.padStart(6) + ' ' + todoTotCost.padStart(6) + C.reset;
  ...
  if (manifest.epics && manifest.epics.length > 0) {
    ...
    for (const epic of manifest.epics) {
      ...
      if (epic.duration) {
        ...
        // Total (color based on pricing mode)
        ...
        epicStats += costColor + eTotTok.padStart(6) + ' ' + eTotCost.padStart(6) + C.reset;
  ...
  // TOTALS line with breakdown: regular (yellow), cached (cyan), total (color based on pricing mode)
  ...
  totalsStats += costColor + formatTokens(totalTokens).padStart(6) + ' ' + formatCost(totalDisplayCost).padStart(6) + C.reset;
