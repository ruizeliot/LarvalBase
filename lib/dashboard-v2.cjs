...
function render() {
  ...
  for (let i = 0; i < LOGO.length; i++) {
    ...
    const versionWidth = (i === LOGO.length - 1) ? 8 : 0; // "  v8.0" visible width
...
function triggerContextRefresh() {
  ...
  const manifest = readManifest();
  const version = manifest?.version || '8.0';  // Read version from manifest for orchestrator command
  if (manifest) {
...
# Step 3: Send /orchestrator-desktop-v${version} AUTO-RESUME-NO-QUESTION
Write-Output "Sending /orchestrator-desktop-v${version} AUTO-RESUME-NO-QUESTION..."
$r3 = [RefreshInjector]::SendText(${ORCHESTRATOR_PID}, "/orchestrator-desktop-v${version} AUTO-RESUME-NO-QUESTION")
Start-Sleep -Milliseconds 800
...
function sendHeartbeat() {
  ...
  if (needsRefresh) {
    ...
    console.log('[HEARTBEAT ' + heartbeatCount + '] Triggering /clear + /orchestrator-desktop-v' + version + ' AUTO-RESUME-NO-QUESTION...');
