...
// Analysis worker script path
const ANALYSIS_SCRIPT_PATH = path.join(__dirname, 'analyze-worker-transcript.cjs');
...
// Spawn analysis worker in visible window
function spawnAnalysisWorker(sessionId, type, number) {
  ...
  console.log(`[ANALYSIS-WORKER] Spawning for ${type} ${number}, session: ${sessionId}`);

  // Calculate window position (bottom-right, smaller window)
  const screenWidth = 1920; // Default, will be adjusted by conhost
  const screenHeight = 1080;
  const windowWidth = 800;
  const windowHeight = 400;
  const windowX = screenWidth - windowWidth - 20;
  const windowY = screenHeight - windowHeight - 60;

  // Build the command
  const nodeCmd = `node "${ANALYSIS_SCRIPT_PATH}" "${PROJECT_PATH}" "${sessionId}" "${type}" "${number}"`;

  // Spawn in visible conhost window
  const title = `Analysis-${type}-${number}`;
  const proc = spawn('conhost.exe', [
    'cmd.exe', '/c',
    `title ${title} && ${nodeCmd} && echo. && echo Analysis complete. Press any key... && pause >nul`
  ], {
    detached: true,
    stdio: 'ignore',
    windowsHide: false
  ...
  proc.unref();
  console.log(`[ANALYSIS-WORKER] Spawned PID: ${proc.pid}`);
}
...
const DATA_COLS_WIDTH = COL.TIME + 1 + COL.REG + 1 + COL.CACHE + 1 + COL.TOTAL; // 53 (with 3 separators)
...
function render() {
  ...
  for (const phase of phases) {
    ...
    if (status === 'complete' && phaseData.duration) {
      ...
      // Regular column: tokens/% + cost (13 chars)
      ...
      stats += COL_SEP + C.yellow + (regVal.padStart(6) + ' ' + regCost.padStart(6)).padStart(COL.REG) + C.reset;

      // Cached column: tokens/% + cost (13 chars)
      ...
      stats += COL_SEP + C.cyan + (cacheVal.padStart(6) + ' ' + cacheCost.padStart(6)).padStart(COL.CACHE) + C.reset;

      // Total column: tokens/% + cost (13 chars)
      ...
      let totalContent = (totVal.padStart(6) + ' ' + totCost.padStart(6)).padStart(COL.TOTAL);
      if (todoCount > 0) totalContent = (totVal.padStart(6) + ' ' + totCost.padStart(4) + C.dim + '(' + todoCount + ')' + C.green).padStart(COL.TOTAL + 10); // +10 for ANSI
      stats += COL_SEP + C.green + totalContent + C.reset;
    } else if (status === 'running') {
    ...
    const leftPart = cursor + icon + '  ' + phase + '. ' + expandIcon + rowColor + name + C.reset;
    // Pad name to fill nameColWidth (calculated from W - data columns - separators)
    const leftVisible = 2 + 1 + 2 + 3 + 2 + name.length; // cursor + icon + spaces + phase# + expand + name
    ...
    if (isExpanded && hasTodos) {
      ...
      for (let i = 0; i < todos.length; i++) {
        ...
        todoStats += COL_SEP + C.yellow + (todoRegVal.padStart(6) + ' ' + todoRegCost.padStart(6)).padStart(COL.REG) + C.reset;
        todoStats += COL_SEP + C.cyan + (todoCacheVal.padStart(6) + ' ' + todoCacheCost.padStart(6)).padStart(COL.CACHE) + C.reset;
        todoStats += COL_SEP + C.green + (todoTotVal.padStart(6) + ' ' + todoTotCost.padStart(6)).padStart(COL.TOTAL) + C.reset;

        // Cursor for todo
        const todoCursor = isCursor(todoKey) ? C.cyan + '>' + C.reset + ' ' : '  ';

        // Name fills space: cursor(2) + indent(4) + prefix(2) + expand(2) + name
        const todoLeft = todoCursor + '    ' + C.dim + prefix + C.reset + ' ' + todoExpandIcon + todo.content;
        const todoLeftVisible = 2 + 4 + 2 + 1 + 2 + todo.content.length;
  ...
  if (manifest.epics && manifest.epics.length > 0) {
    ...
    for (const epic of manifest.epics) {
      ...
      if (epic.duration) {
        ...
        epicStats += COL_SEP + C.yellow + (eRegVal.padStart(6) + ' ' + eRegCost.padStart(6)).padStart(COL.REG) + C.reset;
        ...
        epicStats += COL_SEP + C.cyan + (eCacheVal.padStart(6) + ' ' + eCacheCost.padStart(6)).padStart(COL.CACHE) + C.reset;
        ...
        let eTotalContent = (eTotVal.padStart(6) + ' ' + eTotCost.padStart(6)).padStart(COL.TOTAL);
        if (epic.todoBreakdown) eTotalContent = (eTotVal.padStart(6) + ' ' + eTotCost.padStart(4) + C.dim + '(' + epic.todoBreakdown.length + ')' + C.green).padStart(COL.TOTAL + 10);
        epicStats += COL_SEP + C.green + eTotalContent + C.reset;
      } else {
      ...
      const epicLeftPart = epicCursor + epicIcon + '  ' + epicKey + '. ' + expandIndicator + ' ' + epicRowColor + epic.name + C.reset;
      const epicLeftVisible = 2 + 1 + 2 + 3 + 1 + 1 + epic.name.length; // cursor + icon + spaces + key# + expand + space + name
  ...
  const costPerPercent = '$' + (FIVE_HOUR_SUB_BUDGET / 100).toFixed(2) + ' per %';
  lines.push(line('  ' + C.white + 'PRICING' + C.reset + '       ' + C.dim + costPerPercent + '    Opus $5/$25 /MTok    Cache 10%' + C.reset, W));

  // SUBSCRIPTION line - current usage levels (white label, gray values)
  if (subscriptionUsage) {
    ...
    lines.push(line('  ' + C.white + 'SUBSCRIPTION' + C.reset + '  ' + C.dim + '5-hour: ' + fiveHr + '     7-day: ' + sevenDay + C.reset, W));
  } else {
    lines.push(line('  ' + C.white + 'SUBSCRIPTION' + C.reset + '  ' + C.dim + '(not available)' + C.reset, W));
  }
  ...
  lines.push(line('  ' + C.yellow + '[↑↓]' + C.reset + ' Nav    ' + C.yellow + '[Tab]' + C.reset + ' Expand    ' + C.yellow + '[Space]' + C.reset + ' ' + modeColor + modeLabel + C.reset + '    ' + C.yellow + '[Enter]' + C.reset + ' ' + C.red + '💗' + C.reset + '    ' + C.yellow + '[Q]' + C.reset + ' Quit', W));
  lines.push(C.cyan + '╚' + '═'.repeat(W-2) + '╝' + C.reset);
