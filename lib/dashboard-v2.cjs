...
function getSubCostFromTokens(tokens) {
  ...
  return (estimatedPercent / 100) * WEEKLY_SUB_BUDGET;
}
...
function render() {
  ...
  // Calculate supervisor cost from pipeline usage delta (leftover after workers)
  const pipelineUsageDelta = manifest.pipeline?.usageDelta || 0;
  const supervisorUsageDelta = pipelineUsageDelta > 0 ? Math.max(0, pipelineUsageDelta - totalUsageDelta) : 0;
  const supervisorSubCost = getSubCost(supervisorUsageDelta);
  const hasSupervisorData = pipelineUsageDelta > 0 && supervisorUsageDelta > 0;

  // Grand totals (workers + supervisor)
  const grandUsageDelta = totalUsageDelta + supervisorUsageDelta;
  const grandApiCost = manifest.totalCost || workersApiCost; // Use manifest totalCost if available
  ...
  const workersRegCostVal = formatCost(pricingMode === 'sub' ? getSubCost(workersRegPct) || totalRegularCost : totalRegularCost);
  ...
  const workersCacheCostVal = formatCost(pricingMode === 'sub' ? getSubCost(workersCachePct) || totalCachedCost : totalCachedCost);
  ...
  // SUPERVISOR line - shows orchestrator cost (if pipeline data exists)
  if (hasSupervisorData) {
    const supLeft = '  ' + C.bold + C.magenta + 'SUPERVISOR' + C.reset;
    let supStats = ''.padStart(10) + ' '; // empty time column

    // Supervisor only has total (no regular/cached breakdown available)
    supStats += C.dim + '--'.padStart(6) + '    --' + C.reset + ' ';
    supStats += C.dim + '--'.padStart(6) + '    --' + C.reset + ' ';

    // Total: percentage or placeholder
    const supTotVal = pricingMode === 'sub' ? formatPercent(supervisorUsageDelta) : '--';
    const supTotCost = supervisorSubCost !== null ? formatCost(supervisorSubCost) : '$--';
    supStats += C.magenta + supTotVal.padStart(6) + ' ' + supTotCost.padStart(6) + C.reset;
  ...
  if (hasSupervisorData) {
    // Grand totals only have meaningful total column (reg/cache same as workers)
    ...
    // Total: combined percentage or tokens
    const grandTotVal = pricingMode === 'sub' && grandUsageDelta > 0 ? formatPercent(grandUsageDelta) : formatTokens(totalTokens);
  ...
  if (subscriptionUsage) {
    ...
    lines.push(line('', W));
    lines.push(line('  ' + C.magenta + 'SUBSCRIPTION' + C.reset + '  5hr: ' + fiveHrColor + fiveHr.padStart(4) + C.reset + '  7-day: ' + sevenDayColor + sevenDay.padStart(4) + C.reset + '  cal: ' + C.cyan + calDisplay + C.reset, W));
  }
