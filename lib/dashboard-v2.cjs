...
function captureSessionId() {
  ...
  for (const [phase, data] of Object.entries(manifest.phases)) {
    if (data.status === 'running' && !data.sessionId) {
      ...
      updated = true;
      capturedForRunning = true;
      break; // Only one phase can be running
    }
  }

  // FALLBACK: If no running phase found, try currentPhase from manifest
  // This handles cases where phase was started without orchestrator setting status to running
  if (!capturedForRunning && manifest.currentPhase) {
    ...
    if (!manifest.phases[currentPhase]) {
      ...
      console.log('[SESSION] Created missing phase ' + currentPhase + ' entry');
      updated = true;
    }

    // Capture sessionId if not already set
    if (!manifest.phases[currentPhase].sessionId) {
      manifest.phases[currentPhase].sessionId = sessionId;
      manifest.phases[currentPhase].status = 'running'; // Mark as running
      console.log('[SESSION] Captured sessionId for currentPhase ' + currentPhase + ': ' + sessionId);
      updated = true;
    }
  }
