...
function injectConsole(text, sendEnter = false) {
  return new Promise((resolve) => {
    // Read PowerShell PID (the shell process that owns the console) - NOT the Claude PID
    const psPidPath = path.join(PROJECT_PATH, '.pipeline', 'orchestrator-powershell-pid.txt');
    let orchestratorPid = null;

    if (fs.existsSync(psPidPath)) {
      try {
        const content = fs.readFileSync(psPidPath, 'utf8').trim().replace(/^\uFEFF/, ''); // Remove BOM
        orchestratorPid = parseInt(content, 10);
      } catch (e) {
        // Fall through to manifest/argv fallback
      }
    }

    // Fallback to manifest or command-line arg
    if (!orchestratorPid) {
      const manifest = readManifest();
      orchestratorPid = manifest?.workers?.current?.orchestratorPid || ORCHESTRATOR_PID;
    }
...
async function sendHeartbeat(manual = false) {
  ...
  // Inject heartbeat message (text only, no Enter yet)
  ...
  console.log(`[HEARTBEAT ${heartbeatCount}] Text: ${result1}`);

  // Wait for text to be fully processed before sending Enter
  await new Promise(r => setTimeout(r, 500));

  // Now send Enter separately
  const result2 = await injectConsole('', true);
  console.log(`[HEARTBEAT ${heartbeatCount}] Enter: ${result2}`);

  // Log to file for debugging
  const logPath = path.join(PROJECT_PATH, '.pipeline', 'heartbeat-debug.log');
  fs.appendFileSync(logPath, `[${new Date().toISOString()}] Text: ${result1}\n[${new Date().toISOString()}] Enter: ${result2}\n\n`);
